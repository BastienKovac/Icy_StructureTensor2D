...
/**
 * <p>
 * Apply the structure tensor algorithm in 2D of the PRIMO team of the ITAV
 * (Toulouse, FRANCE) This is a plugin for the software Icy To use it, you have
 * to get a 8-bits image in 2D You have to fix the parameters sigma and delta
 * Choose the representation of the direction of the spheroid Do not hesitate to
 * do some different test with different parameters
 * </p>
 * <p>
 * In case you use this algorithm, please cite : WEISS Pierre, FEHRENBACH
 * Jerôme, DE BRITO Guillaume - ITAV (Toulouse, France)
 * </p>
 * 
 * @see <a href=
...
public class StructureTensor2D extends EzPlug implements Block {
	...
	EzVarBoolean varEllipse = new EzVarBoolean("Ellipse : ", true), varSegment = new EzVarBoolean("Segment : ", false),
			...
			if (choiceRoi == true && (se.getROIs().isEmpty() && varROIs.size() == 0)) {
				...
				if (delta == 0) {
					...
					if (sigma == 0) {
						...
						List<double[]> lShapParam = paramProcessing(se, img, sigma, delta, choiceRoi,
								choiceAnisoDisplay);
						if (choiceEllipse) {
	...
	public List<double[]> paramProcessing(Sequence se, IcyBufferedImage img, int sigma, int delta, boolean choiceRoi,
			boolean choiceAnisoDisplay) {
		...
		if (choiceRoi == true) {
			...
			if (listRoi.size() > 1) {
				for (int i = 1; i < listRoi.size(); i++) {
					bm.add(listRoi.get(i).getBooleanMask2D(0, 0, 0, true));
				}
			}
		} else {
	...
	public void segmentProcessing(Sequence se, List<double[]> lShapParam) {
		...
		// alpha=d[4] ;
		//
		// double[] coordTmp = Segment2DTensor.calculParamLine(a, x0,
		// y0,Integer.parseInt(varDelta.getValue())) ;
		//
		// x1=coordTmp[0];
		// y1=coordTmp[1] ;
		// x2=coordTmp[2] ;
		...
		//
		// Color c = new Color((int)d[6],0,(int)d[5]) ;
		// Segment2DTensor l = new Segment2DTensor((int)x0,(int)
		// y0,(int)x1,(int)y1,(int)x2,(int)y2,alpha,c) ;
